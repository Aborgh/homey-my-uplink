...
export class PowerCalculator {
    ...
    async updateDevicePower(device) {
        try {
            ...
            // Prefer direct system power reading (reported by heat pump in kW)
            const systemPowerKW = device._internalValues?.get('measure_power.system');
            if (systemPowerKW != null && systemPowerKW > 0) {
                power = systemPowerKW * 1000;
                device.log(`Using system power consumption: ${power.toFixed(2)} W (${systemPowerKW} kW)`);
            }

            // Fall back to current-based calculation
            if (power === 0 && hasValidCurrent) {
            ...
            if (power === 0) {
                const lifetimeEnergy = device._internalValues?.get('meter_power.lifetime_energy_consumed')
                    ?? device.getCapabilityValue('meter_power.lifetime_energy_consumed');
